...
REPOS=("GitOps" "python-app" "python-app-1" "lab-go-cli" "Argocd-solutions")
...
verify_repo() {
    local repo_name=$1
    local repo_path="/Users/jaime.henao/arheanja/Backstage-solutions/backstage-app/Repos-portfolio/$repo_name"
...
for file in "${gitops_files[@]}"; do
    if [ -f "/Users/jaime.henao/arheanja/Backstage-solutions/backstage-app/Repos-portfolio/GitOps/docs/$file" ]; then
...
for image in "${gitops_images[@]}"; do
    if [ -f "/Users/jaime.henao/arheanja/Backstage-solutions/backstage-app/Repos-portfolio/GitOps/site/images/$image" ]; then
...
python_app_lines=$(wc -l < "/Users/jaime.henao/arheanja/Backstage-solutions/backstage-app/Repos-portfolio/python-app/docs/index.md" 2>/dev/null || echo "0")
python_app1_lines=$(wc -l < "/Users/jaime.henao/arheanja/Backstage-solutions/backstage-app/Repos-portfolio/python-app-1/docs/index.md" 2>/dev/null || echo "0")

if [ "$python_app_lines" -gt 50 ]; then
    echo -e "   ${GREEN}✅ python-app documentación mejorada ($python_app_lines líneas)${NC}"
else
    echo -e "   ${RED}❌ python-app documentación insuficiente ($python_app_lines líneas)${NC}"
    all_good=false
fi

if [ "$python_app1_lines" -gt 50 ]; then
    echo -e "   ${GREEN}✅ python-app-1 documentación mejorada ($python_app1_lines líneas)${NC}"
else
    echo -e "   ${RED}❌ python-app-1 documentación insuficiente ($python_app1_lines líneas)${NC}"
    all_good=false
fi
